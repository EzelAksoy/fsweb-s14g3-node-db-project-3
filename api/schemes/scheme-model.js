...
async function findById(scheme_id) {
  ...
  */
  const ShemaSteps = await db("schemes as sc")
    .leftJoin("steps as st", "sc.scheme_id", "st.scheme_id")
    .select("sc.scheme_name", "st.*")
    .where("sc.scheme_id", scheme_id)
    .orderBy("st.step_number", "ASC");
  if (ShemaSteps.length === 0) {
    return null;
  }

  const responseData = {
    scheme_id: scheme_id,
    scheme_name: ShemaSteps[0].scheme_name,
    steps: [],
  };
  if (ShemaSteps[0].scheme_id === null) {
    return responseData;
  } else {
    ShemaSteps.forEach((element) => {
...
async function findSteps(scheme_id) {
  ...
  */
  const ShemaSteps = await db("schemes as sc")
    .leftJoin("steps as st", "sc.scheme_id", "st.scheme_id")
    .select("sc.scheme_name", "step_number", "instructions", "step_id")
    .where("sc.scheme_id", scheme_id)
    .orderBy("st.step_number", "ASC");
  if (!ShemaSteps[0].step_id) {
    return [];
  } else {
    ShemaSteps;
  }
...
async function add(scheme) {
  ...
  */
  const addScheme = await db("schemes").insert(scheme);
  return await findById(addScheme);
}
